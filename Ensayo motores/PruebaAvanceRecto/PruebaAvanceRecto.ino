...
// ================================================================
//  FUNCIÓN PID AISLADA PARA REUTILIZAR EN LAS 4 RUEDAS
// ================================================================
int calcular_pwm_rueda(float v_deseada, float v_actual, double &memoria_integral, int pwm_base_motor) {
  // Si ordenamos parar, reseteamos memoria y cortamos corriente
  if (v_deseada == 0.0) {
    memoria_integral = 0;
    return 0;
  }
  ...
  // Anti-Windup adaptado a cada motor
  ...
  // Lógica solo hacia adelante (con freno por inercia si se pasa)
  if (v_deseada > 0.0) { 
    if (Output > 0) {
      pwm_final = pwm_base_motor + (int)Output;
    } else {
      pwm_final = 0; // Freno seguro
    }
  }
...
void setup() {
  ...
  // Configurar las direcciones físicas para avanzar recto
  digitalWrite(FL_DIR, LOW);
  digitalWrite(RL_DIR, LOW);
  digitalWrite(FR_DIR, HIGH); // Motores derechos van invertidos
...
void loop() {
  ...
  if ((ahora_us - lastWindowUs) >= ventana_us) {
    ...
    // 1. GENERAR PERFIL DE VELOCIDADES (5 segundos)
    unsigned long tiempo_desde_inicio = millis() - t_inicio_prueba;
    
    if (tiempo_desde_inicio < 5000) {
      vel_deseada_ms = 0.05; // 0.3 m/s para asegurar un arranque fluido
    } else {
      vel_deseada_ms = 0.0;
    ...
    // 4. APLICAR PID A CADA RUEDA
    int pwm_FL = calcular_pwm_rueda(vel_deseada_ms, v_FL, errSum_FL, base_FL);
    int pwm_FR = calcular_pwm_rueda(vel_deseada_ms, v_FR, errSum_FR, base_FR);
    int pwm_RL = calcular_pwm_rueda(vel_deseada_ms, v_RL, errSum_RL, base_RL);
    int pwm_RR = calcular_pwm_rueda(vel_deseada_ms, v_RR, errSum_RR, base_RR);

    // 5. ESCRIBIR PWM
    ledcWrite(FL_PWM, pwm_FL);
    ledcWrite(FR_PWM, pwm_FR);
    ledcWrite(RL_PWM, pwm_RL);
    ledcWrite(RR_PWM, pwm_RR);

    // 6. IMPRIMIR DATOS (Solo la referencia y la media para no saturar el Plotter)
    float v_media = (v_FL + v_FR + v_RL + v_RR) / 4.0;
    
    Serial.print("Ref:"); Serial.print(vel_deseada_ms, 3); Serial.print(" ");
    Serial.print("V_Media:"); Serial.println(v_media, 3);
  }
}