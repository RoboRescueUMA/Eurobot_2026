...
class ZenitalCameraPublisher(Node):
    def __init__(self):
        ...
        self.get_logger().info(f'Conectando a camara IP: {self.video_url}')
        self.cap = cv2.VideoCapture(self.video_url)

        if not self.cap.isOpened():
            self.get_logger().error('No se puede conectar a la camara IP')
            self.get_logger().error(f'   Verifica que la URL sea correcta: {self.video_url}')
            self.get_logger().error('   Verifica que la app IPCamera este activa en el movil')
            return
    ...
    def read_camera_thread(self):
        """Bucle de lectura rapida para vaciar buffer de la camara"""
        while self.running and rclpy.ok():
            ...
            if ret:
                ...
                self.get_logger().warn('Fallo al leer frame, reintentando...')

    def publish_callback(self):
        """Publica el ultimo frame disponible"""
        frame_to_send = None
        with self.lock:
        ...
        if frame_to_send is not None:
            ...
            try:
                ...
                self.get_logger().error(f'Error en conversion RAW: {e}')
            ...
            try:
                ...
                self.get_logger().error(f'Error en compresion: {e}')
...
def main(args=None):
    ...
    try:
        ...
        node.get_logger().info('Deteniendo camara...')
    finally:
        node.destroy_node()
        rclpy.shutdown()
