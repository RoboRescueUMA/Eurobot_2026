...
class EstadoRobot(Enum):
    ...
    M1_EMPUJAR_AL_NIDO = 4
    
    # --- Misión 2: Secuencia Expandida ---
    M2_EVALUAR_PIEZA = 5
    M2_IR_PUNTO_PREPARACION = 6
    M2_BAJAR_GARRA = 7
    M2_ESPERAR_BAJAR = 8
    M2_ABRIR_GARRA = 9
    M2_ESPERAR_ABRIR = 10
    M2_APROXIMAR_PIEZA = 11
    M2_CERRAR_GARRA = 12
    M2_ESPERAR_CERRAR = 13
    M2_SUBIR_GARRA = 14
    M2_ESPERAR_SUBIR = 15
    M2_RETROCEDER_INTERMEDIO = 16
    M2_IR_PUNTO_ENTREGA = 17
    M2_SOLTAR_PIEZA = 18
    M2_ESPERAR_SOLTAR = 19
    
    RETORNO_EMERGENCIA = 20
    FIN_PARTIDA = 21
...
class CerebroEurobot(Node):
    def __init__(self):
        ...
        self.estado_anterior = None  # Variable para gestionar los logs limpios
    ...
    def calcular_waypoints(self):
        ...
        if self.equipo == "AMARILLO":
            ...
            self.lista_piezas = [
                {"prep_x": 20.0, "prep_y": 60.0, "cap_x": 20.0, "cap_y": 70.0, "ent_x": 15.0, "ent_y": 170.0},
                {"prep_x": 20.0, "prep_y": 60.0, "cap_x": 20.0, "cap_y": 75.0, "ent_x": 15.0, "ent_y": 170.0},
                {"prep_x": 20.0, "prep_y": 60.0, "cap_x": 20.0, "cap_y": 80.0, "inter_x": 40.0, "inter_y": 80.0, "ent_x": 15.0, "ent_y": 80.0},
                {"prep_x": 20.0, "prep_y": 60.0, "cap_x": 20.0, "cap_y": 85.0, "inter_x": 40.0, "inter_y": 100.0, "ent_x": 15.0, "ent_y": 100.0}
            ]
...
# ================================================================
    #  HERRAMIENTAS DE MOVIMIENTO INTEGRADAS (Diferencial)
    # ================================================================
    ...
    def ir_a_punto_como_tanque(self, target_x, target_y, sentido=1):
        ...
        # Si estamos suficientemente cerca, paramos y devolvemos éxito
        if distancia < 8.0: 
            self.parar_motores()
            return True

        # 2. Ángulo al objetivo
        angulo_obj = math.degrees(math.atan2(error_y, error_x))
        if sentido == -1: 
            angulo_obj = self.normalizar_angulo(angulo_obj + 180.0)
            
        error_angular = self.normalizar_angulo(angulo_obj - self.robot_pose.theta)

        # 3. Lógica Híbrida: Girar o Avanzar+Corregir
        vx = 0.0
        w = 0.0
        
        # Si el error angular es grande (más de 15 grados), priorizamos rotar sobre el propio eje.
        # No avanzamos, solo giramos como un tanque.
        if abs(error_angular) > 15.0:
            w = max(VEL_MIN_GIRO, min(VEL_MAX_GIRO, abs(0.03 * error_angular)))
            w = w if error_angular > 0 else -w
            
        # Si el error es pequeño (menos de 15 grados), encaramos el punto. 
        # Avanzamos y corregimos el ángulo SIMULTÁNEAMENTE.
        else:
            # Velocidad lineal proporcional a la distancia
            vx = max(VEL_MIN_RECTO, min(VEL_MAX_RECTO, abs(0.02 * distancia)))
            vx = vx * sentido
            
            # Velocidad angular proporcional al error (Corrección suave en marcha)
            # Como vamos avanzando, permitimos que w oscile de forma natural sin parar el robot
            w = 0.015 * error_angular
            # Opcional: limitar también el giro en marcha si fuera necesario
            # w = max(-VEL_MAX_GIRO, min(VEL_MAX_GIRO, w)) 

        # 4. Enviar comandos a los motores
        self.enviar_velocidad(vx, 0.0, w)
        return False
    ...
    def maquina_de_estados_loop(self):
        # 1. Log de cambio de estado (sin saturar la consola)
        if self.estado_actual != self.estado_anterior:
            self.get_logger().info(f"---> CAMBIO DE ESTADO: {self.estado_actual.name}")
        ...
        if self.estado_actual == EstadoRobot.ESPERANDO_TIRETTE:
            ...
            if self.ir_a_punto_como_tanque(self.m1_pos_x, self.m1_pos_y, -1): self.estado_actual = EstadoRobot.M1_EMPUJAR_AL_NIDO
        
        elif self.estado_actual == EstadoRobot.M1_EMPUJAR_AL_NIDO:
            # FIX: El robot pasará a la Misión 2 si la función de tanque dice que ha llegado (True)
            # O si detecta por fuerza bruta que ha cruzado la coordenada Y (redundancia de seguridad).
            llegado_al_punto = self.ir_a_punto_como_tanque(self.park_x, self.park_y, -1)
            if llegado_al_punto or (self.robot_pose is not None and self.robot_pose.y >= ZONA_ENTREGA_Y):
                self.parar_motores()
        ...
        # ==========================================================
            ...
            self.estado_actual = EstadoRobot.M2_ESPERAR_SOLTAR

        elif self.estado_actual == EstadoRobot.M2_ESPERAR_SOLTAR:
            if self.accion_garra_completada:
