...
from geometry_msgs.msg import Pose2D, TransformStamped  # Usamos Pose2D para tener X, Y y Theta
...
# Funcion para convertir de Euler a Cuaternion
...
class EurobotVisionNode(Node):
    def __init__(self):
        ...
        # Broadcaster de transformadas para Nav2
        ...
        self.base_dir="/mnt/c/Users/Lucia/Downloads/Software_Pruebas/Visión Arucos/src/eurobot_vision/eurobot_vision"
        ...
        self.GROUND_MARKERS_CENTERS = {
            20: np.array([2.4, 0.6, 0.0]),
            21: np.array([0.6, 0.6, 0.0]), 
            22: np.array([2.4, 1.4, 0.0]), 
            23: np.array([0.6, 1.4, 0.0]), 
}
                ...
                for i, mid in enumerate(ids_flat):
                    if mid == self.ROBOT_ID:
                        ...
                        if pos_w is not None and pos_top_w is not None and pos_bottom_w is not None:
                            ...
                            # Publicar transformada para Nav2
                            t = TransformStamped()
                            t.header.stamp = self.get_clock().now().to_msg()
                            t.header.frame_id = "map"
                            t.child_frame_id = "base_link"

                            # Asignamos la posicion en x e y
                            t.transform.translation.x = float(pos_w[0])
                            t.transform.translation.y = float(pos_w[1])
                            t.transform.translation.z = 0.0  # El robot está en el suelo

                            # Convertimos el ángulo a cuaternion
